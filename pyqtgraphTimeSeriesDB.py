    ...
    * space begins playing frames. If time values (in seconds) are given 
    ...
    def __init__(self, parent=None, name="ImageView", view=None, imageItem=None, *args):
        ...
        **Arguments** 
        ...
        self.aroicolumns = ['Color', 'Type', 'Z:XY']
    ...
    def importimage(self, imagefile):
        ...
        print(imagefile)
        return tifffile.imread(str(imagefile))
    ...
    def loadImage(self, pathfile):
        print('loadimagestarted------------------------------------------------------------------')
        print(pathfile)
        self.placeHolderPath, filename = os.path.split(str(pathfile))
        lb = loadImageDialogue.ImageChoice(workingdirectory = self.placeHolderPath, filename=filename)
        returnCode=lb.exec_()
        print(returnCode)
        value = lb.GetValue()
        ...
        if not '.hdf5' in pathfile:
            #load first image
            image = self.importimage(pathfile)
            ...
            imgRGB_file = tables.openFile(pathfile, mode='r')
        ...
        if os.path.isfile(self.fileROI):
            maskdata = np.load(self.fileROI)
            maskdata = maskdata.item()
            columns = ('Z:XY', 'Color', 'Type')
            maskdata2 = {}
            for ckey in columns:
                maskdata2[ckey] = maskdata[ckey]
        #unkown reason {ckey : maskdata[ckey] for ckey in columns} cannot find maskdata
            self.aroi = pd.DataFrame(maskdata2)
        else:
            self.aroi = pd.DataFrame(columns=self.aroicolumns)
        #pdb.set_trace()
        #HDF5 = ables.carray.CArray isinstance(imgRGB, tables.carray.CArray) == True
        if isinstance(imgRGB, np.ndarray):
            imgRGB = np.rollaxis(imgRGB, 3, 2)
        self.setImage(imgRGB,  axes = {'t': 0, 'z':1, 'x':2, 'y':3, 'c':4}, fileROI= self.fileROI)
    ...
    def saveROI(self):
        ...
        #print(w1.aroi)
        mask_index = {} #dictionary containing all binary images
        intensity_data = {}
        for croi in self.aroi.index:
            mask = np.zeros(self.image.shape[1:4], dtype = np.bool)
            for layer in self.aroi['Z:XY'].loc[croi].keys():
                if len(self.aroi['Z:XY'].loc[croi][layer]) == 1:
                    ...
                    mask[layer,  yy, xx] = 1
            mask_index[croi] = np.where(mask.flatten())
            #get data for roi
    ...
    def createROI(self, newroi, type):
        ...
        if self.currentRowSelectedinTable == None:
            testname = 0
            while str(testname) in self.aroi.index:
                testname += 1
            testname = str(testname)
            #[Color', 'Type', 'Z:XY']
            self.aroi.loc[testname] = ['r', type, {self.currentLayer : self.croi}] 
        else:
    ...
    def quickMinMax(self, data):
        ...
        '''
        print('Getting max and minimum values')
        if data.size > 1e6:
            interval = data.size / 1e6
            ax = np.argmax(data.shape)
            sl = [slice(None)] * data.ndim
            sl[ax] = slice(None, None, interval)
            sl = tuple(sl)
            data = data[sl]
        print('Finished getting max and minimum values')
        return nanmin(data), nanmax(data)
